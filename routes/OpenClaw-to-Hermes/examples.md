...
그리고 export가 끝났다는 말을 먼저 짧게 알려주고,
복잡한 importer 프롬프트 대신 pack 경로, OHL GitHub 주소,
그리고 대상 쪽에서 "OHL 깔고 import 하고 싶다"고 말하면 된다는 다음 단계만 간단히 알려줘.
...
After export is done, prefer a short visible handoff:
- say the export is finished
- show the pack path
- show the OHL GitHub URL
- tell the owner they can go to the target side, install OHL, and say they want to import
```
